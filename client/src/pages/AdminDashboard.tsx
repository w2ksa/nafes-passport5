...
export default function AdminDashboard() {
  ...
  const handleAddStudent = async () => {
    ...
    try {
      ...
      setGradeFilter(grade); // أظهر الصف الذي تمت إضافة الطالب إليه
      toast.success(`تم إضافة الطالب بنجاح في الصف ${grade === 3 ? "الثالث" : "السادس"}`);
    } catch (error) {
  ...
  // معالجة بيانات الإكسل المرفوعة (تحديث قاعدة البيانات بدلاً من العرض فقط)
  const handleExcelDataLoaded = async (uploadedStudents: Student[]) => {
    toast.info(`جاري تحديث وتخزين درجات ${uploadedStudents.length} طالب، هذي العملية ممكن تاخذ شوية وقت...`);
    let updatedCount = 0;
    
    // نقارن الطلاب المرفوعين مع الموجودين بقاعدة البيانات بالاسم
    for (const upStd of uploadedStudents) {
      const existing = students.find(s => s.name.trim() === upStd.name.trim());
      if (existing) {
        try {
          const totalP = calculateTotalPoints(upStd.points, existing.grade);
          const r = getRankByPoints(totalP);
          ...
          console.error("Error updating", existing.name, e);
        }
      }
    }
    
    if (updatedCount > 0) {
      toast.success(`تم بنجاح حفظ وتحديث درجات ${updatedCount} طالب في قاعدة البيانات ✅`);
      // Since useStudentsRealtime is active, the table will refresh automatically
    } else {
      toast.warning("ماتم تحديث درجات أحد! تأكد إن أسماء الطلاب بالإكسل تطابق أسماء المنصة بالضبط.");
    }
  };
      ...
      <main className="flex-1">
              ...
              <div className="flex items-center gap-3">
                ...
                {isUnlocked && (
                  <Dialog open={isAddDialogOpen} onOpenChange={(open) => {
                      ...
                      if (open) setNewStudent({ name: "", grade: "6" }); // reset عند كل فتح
                    }}>
                      ...
                      <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2">
                            <button
                              type="button"
                              onClick={() => setNewStudent(prev => ({ ...prev, grade: "6" }))}
                            ...
                            <button
                              type="button"
                              onClick={() => setNewStudent(prev => ({ ...prev, grade: "3" }))}
                              className={`flex-1 py-2 px-4 rounded-lg border text-sm font-medium transition-colors ${
                                newStudent.grade === "3"
                                  ? "bg-primary text-primary-foreground border-primary"
                                  : "bg-background border-border hover:bg-muted"
        ...
        {isUnlocked && (
          <section className="py-8">
            <div className="container">
              <ExcelUploader isUnlocked={isUnlocked} onDataLoaded={handleExcelDataLoaded} students={students} />
            </div>
          </section>
        )}
